...
const NAV_LINKS_MOBILE_COMITIVA = [
  ...
  { href: "#convidados-honra", label: "Convidados de Honra", sectionId: "convidados-honra" },
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          href="#home"
          className={cn(
            "font-heading text-xl font-semibold transition-colors min-h-[44px] min-w-[44px] flex items-center",
            activeSection === "home" ? "text-sage" : "text-brown"
          )}
        ...
        <div className="hidden lg:flex items-center gap-6">
          {NAV_LINKS_DESKTOP.map((link) => {
            const isActive =
              ...
              (link.sectionId === "padrinhos" && isComitivaActive);
            return (
              <Link
                key={link.href}
                href={link.href}
          ...
          >
            <div className="flex flex-col gap-1 pt-8">
              ...
              <div className="mt-2">
                <button
                  type="button"
                  onClick={() => setComitivaExpanded(!comitivaExpanded)}
                  className={cn(
                ...
                >
                  Nossa Comitiva
                  <ChevronDown
                    className={cn("size-5 transition-transform", comitivaExpanded && "rotate-180")}
                  />
                </button>
                <div
                  id="comitiva-submenu"
                  className={cn(
                    ...
                    comitivaExpanded ? "max-h-40 opacity-100" : "max-h-0 opacity-0"
                  )}
                >
